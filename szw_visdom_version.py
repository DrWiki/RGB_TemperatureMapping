...
if __name__ == '__main__':
    ...
    csvfilek = open(f"{folder}/K_{name}.csv", "w",encoding='utf8',newline='')
    writerk = csv.writer(csvfilek)
    writerk.writerow(["frame_num", "K"])
    ...
    while True:
        ...
        if last_frame.__len__()==Time_int and laplace_x is not None:
            ...
            #vis.line([np.sum(np.sum(kpc,0),0)], [frame_num], win='K', update='append')
            writerk.writerows([[frame_num, np.sum(np.sum(kpc,0),0)]])
