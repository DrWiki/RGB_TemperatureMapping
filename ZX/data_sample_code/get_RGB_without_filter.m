...
%     x = round(x);
%     y = round(y);
%     disp(['xy坐标值: (', num2str(x), ', ', num2str(y), ')'])
    
    % 记录到RGB向量中
    R(Tem-14) = I0(x_v(Tem-14), y_v(Tem-14), 1);
    G(Tem-14) = I0(x_v(Tem-14), y_v(Tem-14), 2);
    B(Tem-14) = I0(x_v(Tem-14), y_v(Tem-14), 3);
    
    % 记录xy坐标值到向量中，i.e.记录选取点
%     x_v(Tem-14) = x;
%     y_v(Tem-14) = y;
...
%% 合并矩阵，写入到表格中
RGBT_xy_record = [R, G, B, Tem_range', x_v, y_v];   % 合并成矩阵
% 每一列分别是R, G, B, T, 坐标点的x, 坐标点的y
...
% 写入csv文件
csvwrite('RGBT_xy_record_without_filter.csv', RGBT_xy_record);
disp('--------------------program end--------------------')