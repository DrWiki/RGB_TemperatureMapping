...
row_center = RGBT_rc_data(:, 5);
col_center = RGBT_rc_data(:, 6);


%% 使用循环采集数据
% 设置偏移的范围，对图像进行分析可知，bias <= 30都可信，<=20绝对稳
row_b_range = -20:40:20;
col_b_range = -20:40:20;

for row_b = row_b_range
    for col_b = col_b_range
        %% 申请空间
        ...
        % 本次的采样点坐标 = 中心点坐标 + 偏移量
        row_v = row_center + row_b;
        col_v = col_center + col_b;
        % 循环，对每一个温度提取一个RGB值
        for Tem = Tem_range
            %% 图像读取与滤波
        ...
        %     disp(['coly坐标值: (', num2str(col), ', ', num2str(row), ')'])
            ...
            % 记录到RGB向量中
            R(Tem-14) = I(row_v(Tem-14), col_v(Tem-14), 1);
            G(Tem-14) = I(row_v(Tem-14), col_v(Tem-14), 2);
            B(Tem-14) = I(row_v(Tem-14), col_v(Tem-14), 3);
        ...
        %% 合并矩阵，写入到表格中
        RGBT_rc_record = zeros(size(RGBT_rc_data)); % 先清空为0
        ...
        % 写入csv文件
        csvwrite(['..\data\RGBT_rc_record_bias_',num2str(row_b),...
            '_', num2str(row_b), '.csv'], RGBT_rc_record);
    end
end
